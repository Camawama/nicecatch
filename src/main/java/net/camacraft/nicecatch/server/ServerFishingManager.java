...
public class ServerFishingManager
{
                                         ...
                                         InteractionHand hand, PathfinderMob fish)
    {
        ...
        beginFight(player, session, fight, fish, cfg.rodRangeLimit.get() - 1.0D);
    }
    ...
    private static void beginFight(ServerPlayer player, Session session, FishFight fight, PathfinderMob fish, double maxLine)
    {
        ...
        // the reel's spool (a rod fight stays under line.rodRangeLimit, where the line gives out
        // entirely; an arrow reel carries its own line).
        ...
        if (headroom < 8.0D || fight.weightLbs < cfg.soundPhaseMinWeightLbs.get()) {
            ...
            // escape is even on the table. A basic-reel (bottom-tier) fish also opens
            // thrashing — it never runs at all, so no hook-set run for it either.
    ...
    private static void tickFight(ServerPlayer player, Session session, FishingHook hook)
    {
        ...
        if (--fight.phaseTicks <= 0) {
            advancePhase(player, fight, random, level, fish);
        }
        ...
        // A fresh hookup gets a moment of forgiveness: crank-tension gains ramp in over the
            ...
            if (run) {
                ...
                if (charging) {
                    ...
                    fight.tension += (crank * cfg.tensionPerRevolutionRun.get().floatValue()
                            * (0.4F + 0.6F * fight.strength)
                            + 0.004F * (0.5F + fight.strength)) / fight.tensionScale * tensionEase;
                } else {
                ...
                // Calm phases are the DESIGNED reel-hard windows: even against the strongest
                // fish, sustained full-overdrive cranking must roughly break even with the
                // recovery — the calm crank tax exists to punish sloppiness, not the one
                // play the fight demands. (The old 0.5+strength factor red-lined a monster
                // in five seconds of doing exactly the right thing.)
                fight.tension += crank * cfg.tensionPerRevolutionCalm.get().floatValue()
                        * (0.35F + 0.45F * fight.strength) / fight.tensionScale * tensionEase;
                fight.tension -= cfg.tensionRecoveryPerTick.get().floatValue();
            }
        } else {
                                      ...
                                      ServerLevel level, RandomSource random, float side)
    {
        ...
        if (fight.dartTicksLeft > 0) {
            ...
            if (fight.crankQuietTicks >= DART_CRANK_QUIET_TICKS) {
                float correct = fight.dartDir == 1 ? Math.max(0.0F, -side) : Math.max(0.0F, side);
                // Ease into the pull: only a smooth, sustained sweep counts, capped per tick.
                // A violent single-tick yank wastes most of its motion — and past a little
                // headroom it strains the line, the way snatching a real rod sideways would.
                float cap = cfg.dartMaxPullPerTick.get().floatValue();
                fight.dartProgress += Math.min(correct, cap);
                float excess = correct - cap * 1.4F;
                if (excess > 0.0F) {
                    fight.tension = Mth.clamp(fight.tension
                            + excess * cfg.dartJerkTensionPenalty.get().floatValue()
                            / fight.tensionScale, 0.0F, 1.0F);
                }
            }
        ...
        // Darts are a heavyweight's move: light fish never spring them, so everyday catches
     ...
     */
    private static void advancePhase(ServerPlayer player, FishFight fight, RandomSource random,
                                     ServerLevel level, PathfinderMob fish)
    {
    ...
    /**
     * The fish's rung on the difficulty ladder, for the HUD's coaching: 0 = basic reel only,
     * 1 = works the rod vertically (sounding), 2 = also charges, 3 = also darts.
     */
    private static byte fightTier(FishFight fight)
    {
        NiceCatchConfig.Server cfg = NiceCatchConfig.SERVER;
        if (fight.weightLbs >= cfg.dartMinWeightLbs.get()) return 3;
        if (fight.weightLbs >= cfg.chargePhaseMinWeightLbs.get()) return 2;
        if (fight.weightLbs >= cfg.soundPhaseMinWeightLbs.get()) return 1;
        return 0;
    }
    ...
    private static FightPhase pickPhase(FishFight fight, RandomSource random, FightPhase prev)
    {
        ...
        // Bottom rung of the difficulty ladder: a basic-reel fish NEVER runs. Runs are the
        // mechanic that takes line, and a pan fish takes no line — it thrashes and holds
        // while the crank brings it straight home, easy from hook-set to hand.
        if (fight.weightLbs < NiceCatchConfig.SERVER.soundPhaseMinWeightLbs.get()) {
            return random.nextFloat() < 0.55F ? FightPhase.HOLD : FightPhase.SWEEP;
        }
        ...
        // The difficulty ladder trims the tactic pool by weight: light catches stay a pure
        // crank-and-reel affair (no Sounding dives), and only mid-weights and up spring the
                                       ...
                                       boolean run, float crank, float lift)
    {
        ...
        if (fight.holding && crank > 0.001F && dist > 1.2D) {
            ...
            double runEase = 1.35D - 0.35D * fight.strength;
    ...
    /**
     * The cut-line keybind: the angler gives up on whatever the line is doing — a fight
     * that's too much fish, a bite, a retrieve, or just a parked bobber — and snips it.
     * The fish (if any) bolts free, the bobber and hook are lost with the line, and the
     * rod takes no damage: walking away costs the terminal tackle, nothing more.
     */
    public static void handleCutLine(ServerPlayer player)
    {
        Session session = session(player);
        ServerLevel level = player.serverLevel();
        FishingHook hook = player.fishing;
        if (session.fight == null && session.retrieve == null && hook == null) return;

        if (session.fight != null) {
            unhookFish(level, session.fight);
            if (hook != null) hook.setHookedEntity(null);
        }
        session.retrieve = null;
        clearBiteFlow(level, session);
